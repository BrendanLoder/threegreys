...
export default function Dashboard() {
    ...
    const [doNotWantIds, setdoNotWantIds] = useState([])
    ...
    useEffect(() => {
        ...
        const initialWantItems = wants && wants.length > 0 ? wants.map((want, index) => 
        ...
        setWantItems(initialWantItems)

        const initialDoNotWantItems = doNotWants && doNotWants.length > 0 ? doNotWants.map((doNotWant, index) => 
        ...
        setDoNotWantItems(initialDoNotWantItems)
    ...
    const handleNewWantSubmission = async (event) =>
    {
        ...
        const newUserWantIdList = await addUserWant(newWant)
        // console.log('newUserWantIdList', newUserWantIdList)
    ...
    const handleNewDoNotWantSubmission = async (event) =>
    {
        ...
        const newUserNoNotWantIdList = await addUserDoNotWant(newDoNotWant)
        getDoNotWantItemsByUserId(currentUser.userId)
        // setDoNotWants(newUserNoNotWantList)
    }

    console.log('current user??', currentUser)
    // attempting to reload wants list
    useEffect(
        () => {
            console.log('my currentUser is', currentUser)
